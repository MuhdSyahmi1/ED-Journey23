<x-layouts.app title="HNTEC Programmes">
        ...
        <div class="p-6">
            <div class="max-w-7xl mx-auto space-y-6">
                ...
                <div class="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-200/60 dark:border-slate-700/60 overflow-hidden">
                    <div class="bg-gradient-to-r from-purple-400/90 to-indigo-500/90 text-white py-4 px-6">
                        <div class="flex items-center justify-between">
                            <div>
                                <h1 class="text-2xl font-bold">HNTEC PROGRAMMES</h1>
                                <p class="text-sm opacity-90 mt-1">Manage Higher National Technical programme requirements and configurations</p>
                            </div>
            ...
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Manage HNTEC Programmes</h2>
                <button onclick="document.getElementById('addProgrammeModal').classList.remove('hidden')" 
                        class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-sm font-medium transition-colors duration-200 flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-2xl shadow-lg border border-slate-200/40 dark:border-slate-700/40 p-4 mb-6">
                <form method="GET" action="{{ route('staff.admission.hntec-programmes') }}" class="flex flex-col md:flex-row gap-4">
                    <!-- Search -->
                    <div class="flex-1">
                    ...
                    <div class="flex gap-2 items-end">
                        ...
                        <a href="{{ route('staff.admission.hntec-programmes') }}" 
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($programmes as $programme)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $programme->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <div class="flex gap-2">
                                        <button onclick="editProgramme({{ $programme->id }}, '{{ $programme->name }}')" 
                                        ...
                                        <form method="POST" action="{{ route('staff.admission.hntec-programmes.destroy', $programme->id) }}" 
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-6 py-12 text-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">No HNTEC Programmes</h3>
    ...
    <div id="addProgrammeModal" class="hidden fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50">
        <div class="bg-white/95 dark:bg-slate-800/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-slate-200/60 dark:border-slate-700/60 p-6 w-full max-w-md mx-4">
            ...
            <form method="POST" action="{{ route('staff.admission.hntec-programmes.store') }}" class="space-y-4">
                @csrf
                <div>
    ...
    <div id="editProgrammeModal" class="hidden fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50">
        <div class="bg-white/95 dark:bg-slate-800/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-slate-200/60 dark:border-slate-700/60 p-6 w-full max-w-md mx-4">
            ...
            <form id="editForm" method="POST" class="space-y-4">
                ...
                <div class="flex gap-3 pt-4">
                    <button type="submit" class="flex-1 px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-sm font-medium transition-colors duration-200">
                        Update Programme
                    </button>
    ...
    <script>
        function editProgramme(id, name) {
            ...
            document.getElementById('editForm').action = `/staff/admission/hntec-programmes/${id}`;
