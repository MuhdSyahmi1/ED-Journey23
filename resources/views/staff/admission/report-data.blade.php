<x-layouts.app title="Report Data">
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('User Profile Reports') }}
        </h2>
    </x-slot>
            ...
            <div class="max-w-7xl mx-auto">
                ...
                <div class="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-200/60 dark:border-slate-700/60 overflow-hidden">
                    <div class="bg-gradient-to-r from-blue-400/90 to-purple-500/90 text-white text-center py-4">
                        <h1 class="text-2xl font-bold">USER PROFILE REPORTS</h1>
                        <p class="text-sm opacity-90 mt-1">Generate and export user profile data reports</p>
                    </div>
                </div>
                ...
                <div class="mt-6 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-200/60 dark:border-slate-700/60 p-6">
                    <form method="GET" action="{{ route('staff.admission.report-data') }}" class="space-y-4">
                        <div class="flex flex-col lg:flex-row gap-4">
                            ...
                            <div class="flex items-end gap-2">
                                ...
                                <a href="{{ route('staff.admission.report-data') }}" class="bg-slate-500 hover:bg-slate-600 text-white font-bold py-3 px-6 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200">
                                    <svg class="w-5 h-5 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="mt-6 flex justify-end">
                    <a href="{{ route('staff.admission.report-data.pdf', request()->query()) }}" 
                       class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-red-500 to-pink-600 hover:from-red-600 hover:to-pink-700 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200">
                        ...
                        <table class="w-full">
                            <thead class="bg-slate-50 dark:bg-slate-700/50">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wider">APPLICATION DATE</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wider">APPLICATION STATUS</th>
                                ...
                                @forelse($reportData as $application)
                                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-slate-900 dark:text-slate-100">{{ $application->full_name }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <div class="text-xs text-slate-500 dark:text-slate-400">
                                                {{ \Carbon\Carbon::parse($application->application_date)->format('h:i A') }}
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-slate-900 dark:text-slate-100">{{ $application->email_address }}</div>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-6 py-12 text-center">
                                            <div class="flex flex-col items-center justify-center">
                                                ...
                                                <h3 class="text-lg font-medium text-slate-900 dark:text-slate-100 mb-2">No applications found</h3>
                                                <p class="text-sm text-slate-600 dark:text-slate-400">
                                                    @if(request()->hasAny(['search', 'status']))
                                                        Try adjusting your search criteria or filters.
                                                    @else
                                                        No user profile applications have been submitted yet.
                                                    @endif
                                                </p>
                                            </div>
                                        </td>
